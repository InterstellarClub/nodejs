...
اذا كانت الفقرة السابقة تحتوي على مصطلحات مبهمبة بالنسبة إليك
...
تقوم بتشغيل خادم (server) عبر استدعاءٍ غير متزامن (blocking call) مثل `EventMachine::run()`. ولكن في النود جي اس لا يوجد شيء من هذا القبيل.
...
يجب عليك ان تدرك ان تصميم النود بدون خيوط (threads) لا يعني انك لا تستطيع تحقيق الاستفادة الكاملة من الانوية المتعددة في البيئة التي تعمل عليها، فيمكن توليد المعالجات الفرعية عبر واجهة برمجة التطبيقات [`child_process.fork()`][] الخاصة بنا حيث انها مصممة لتكون سهلة التعامل.
هناك ايضا نموذج [`cluster`][] المبني على نفس الواجهة والذي يسمح لك بمشاركة المآخذ (sockets) بين العمليات لموازنة الحمل على الانويةالخاصة بك.
